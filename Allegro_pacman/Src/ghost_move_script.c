...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	*/
	...
	for (Directions i = 1; i <= 4; i++)
	{
		if (i != counter_one && ghost_movable(ghost, M, i, ghost->status))
		{
			proba[cnt++] = i;
		}
	}
	...
	{ // for the dead end case => 其實是dead end case (所以counter_one的case其實是在這裡handle)
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	*/
	...
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
		if (i!= counter && ghost_movable(ghost, M, i, ghost->status)) 	proba[cnt++] = i;
	if (cnt == 0)
	{
		proba[cnt++] = counter;
	}
	if (cnt == 1 && proba[0] == shortestDirection)
	{
		ghost_NextMove(ghost, proba[0]);
	}
	else if (cnt >= 1)
	{
		if(proba[generateRandomNumber(0, cnt - 1)] != shortestDirection)
			ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt - 1)]);
	}
	*/
	/*
	//為什麼還是有抽搐的問題??? 如果pre跟shortest是依樣的case?
	//CASE 1 => 部會抽搐(在房間裡可以出來)但不太能正確逃跑(在遇到的時候)

	static Directions proba[4]; // possible movement
	if(ghost_movable(ghost, M, counter_one, ghost->status) && counter_one!=counter && (value_timer() !=0))
	...
	else
	{
		...
		{
			//not for loop => since the check direction order is same
			while (generateRandomNumber(0, cnt - 1) >= 0)
			{
				if (proba[generateRandomNumber(0, cnt - 1)] != shortestDirection)
				{
					ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt - 1)]);
	...
	*/
	...
	static Directions proba[4]; // possible movement
		...
		for (Directions i = 1; i <= 4; i++)
			if (i != counter && ghost_movable(ghost, M, i, ghost->status)) 	proba[cnt++] = i;
		if (ghost_movable(ghost, M, counter_one, ghost->status) && cnt)
			ghost_NextMove(ghost, counter_one);
		else if (cnt == 0)
		{
			proba[cnt++] = counter;
		...
		else if (cnt == 1 && proba[0] == shortestDirection)
		{
			ghost_NextMove(ghost, shortestDirection);
		}
	...
	if (!movetime(ghost->speed))
		...
		case GO_IN:
			...
			if (M->map[ghost->objData.Coord.y][ghost->objData.Coord.x] == 'B') 
			{
				...
				al_set_timer_count(game_tick_timer,0); //add
...
void ghost_move_script_shortest_path(Ghost* ghost, Map* M, Pacman* pacman) 
{
	...
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			...
			break;
		case FREEDOM:
			int dis_x = ghost->objData.Coord.x - pacman->objData.Coord.x;
			int dis_y = ghost->objData.Coord.y - pacman->objData.Coord.y;
			if (dis_x*dis_x + dis_y*dis_y <= 9)
			{
