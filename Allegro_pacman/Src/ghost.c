...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	if (ghost->status == FLEE) 
	{
		...
		*/
		int duration = value_duration();
		int timer = value_timer();
			if (timer >= 0.7*duration)
			{
				//printf("timer=%d duration=%d\n",timer,duration);
				int offset = 0;
				...
				al_draw_scaled_bitmap(ghost->flee_sprite, bitmap_x_offset + offset, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
			...
			else 
			{
				...
				if (((ghost->objData.moveCD >> 4) & 1) ==0)
				{
					//game_log("in offset=16\n");
					offset = 16;
				}
				else
				{
					//game_log("in offset=0\n");
					offset = 0;
				}
	...
	else 
	{
		...
		if (((ghost->objData.nextTryMove >> 4) & 1) == 0)
		{
			offset = 0;
		}
		else if (((ghost->objData.nextTryMove >> 4) & 1) == 1)
		{
			offset = 16;
		}
...
void ghost_collided(Ghost* ghost) {
	if (ghost->status == FLEE) {
		ALLEGRO_SAMPLE* eatghost = load_audio("Assets/Music/pacman_eatghost.ogg");
		ALLEGRO_SAMPLE_ID PACMAN_POWERSOUND_ID = play_audio(eatghost, effect_volume);
		ghost->status = GO_IN;
