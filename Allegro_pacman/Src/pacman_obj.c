...
extern ALLEGRO_SAMPLE_ID PACMAN_POWEROUND_ID;
...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	if (pacman_wall && x>=1 && x<=34 && y>=1 && y<=28)//will not run out of the wall
	{
		return true;
	}
...
void pacman_draw(Pacman* pman) {
	...
	//Draw default image =>會疊圖
	/*
	if (!pman->objData.preMove)
	{
	...
	*/
	...
	if (!game_over) {
		...
		//if(pman->objData.moveCD % 2 == 0) //even
		if(((pman->objData.moveCD >> 4) & 1) == 0)
		{
			offset = 0;
		}
		//else if(pman->objData.moveCD % 2 == 1) //odd
	...
	else
	{
		...
		// refer al_get_timer_count and al_draw_scaled_bitmap. Suggestion frame rate: 8fps. (一秒鐘8張圖)
		int deathAnimCount = al_get_timer_count(pman->death_anim_counter);

		//game_log("timer=%d\n", deathAnimCount);
		//deathAnimCount = al_get_timer_count(pman->death_anim_counter);
		al_draw_scaled_bitmap(pman->die_sprite, deathAnimCount * 16, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0);
		/*
		...
		if (frameIndex >= 12)
		{
			// Stop the death animation timer and perform any additional actions
			al_stop_timer(pman->death_anim_counter);
			// Additional actions (e.g., game over screen, reset game, etc.)
		}
		*/
	}
}
...
void pacman_eatItem(Pacman* pacman, const char Item) {
	...
	case 'P':
		...
		PACMAN_POWERSOUND_ID = play_bgm(PACMAN_POWERSOUND, effect_volume);
		break;
...
int offset = 16;
int i = 0;
double frame_duration = 1.0 / 7.5;
while (al_get_timer_count(pman->death_anim_counter) < 16) // 12 張圖，但還不確定回來的狀況(增加的速率是多少，會增加到多少!)
{
	game_log("death=%d\n", al_get_timer_count(pman->death_anim_counter));
	game_log("i=%d\n", i);
	game_log("offset=%d\n", offset);
	al_draw_scaled_bitmap(pman->die_sprite, i + offset, 0,
		16, 16,
		drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
		draw_region, draw_region, 0
	);
